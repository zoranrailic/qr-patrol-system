            ...
            if (isset($content['key'])) {
                if ($content['key'] != '') {
                    if (preg_match($regex, $content['key'])) {
                        ...
                        if ($res && method_exists($res, 'getStatusCode') && $res->getStatusCode() == 200) {
            ...
            foreach ($folders as $iValue) {
                if ($message == 'expired') {
                    if (File::exists($iValue)) {
                        ...
                        // \Log::info("Trebalo bi da se obriše: " . $iValue);
                    }
                }
            }
        } else {
    ...
    public function connectSrv($code)
    {
        if (!self::$client) {
            self::$client = new Client([
                'base_uri' => 'https://connectwithdev.com/purchase/public',
                'timeout'  => 10.0,
                'http_errors' => false,
                'headers' => [
        ...
        $params = [
            'query' => ['code' => $code, 'url' => request()->getHttpHost()],
            'http_errors' => false
        ];
